...
int main(){
    ...
    /*
     Exercício 1
     Execute a busca em largura no grafo dado
     mostrando a distância de cada vértice para a origem S
    */
    ...
    busca_em_largura(grafo, 's');

    print_path(grafo,'s','i');
    return 0;
}
...
void _print_path(vertice * origem, vertice * destino){
    ...
    //se o vértice destino não tiver antecessores
        ...
        printf("Distancia entre %c e %c: %d\n", origem->nome, destino->nome, destino->d);
